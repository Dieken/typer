...
export R=$HOME/tmp/input-method-analysis/yustar/星陳輸入法_v3.10.1/schema
a=$(grep '^\s*-\s\+yuhao/' $R/yustar_sc.dict.yaml | grep -v compatible | perl -lpE 's/\r//; s/^\s*\-\s*/-d $ENV{R}\//; s/$/.dict.yaml/')
