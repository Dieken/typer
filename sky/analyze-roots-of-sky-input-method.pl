...
#
# Usage: analyze-roots-of-sky-input-method.pl [拆分表.txt [单字全码.txt]] | cut -f 2,3 | sort -u
...
while (my ($k, $v) = each %chaifen) {
    ...
    if (@$v == 1) {         # 单根字
        say $k, "\t", $v->[0], "\t", substr($v2, 0, 2);
    } elsif (@$v == 2) {    # 双根字
        say $k, "\t", $v->[1], "\t", substr($v2, 1, 2);
        say $k, "\t", $v->[0], "\t", substr($v2, 0, 1), length($v2) == 4 ? substr($v2, 3, 1) : "v";
    } elsif (@$v == 3) {    # 三根字
        say $k, "\t", $v->[2], "\t", substr($v2, 2, 2);
    }
}
